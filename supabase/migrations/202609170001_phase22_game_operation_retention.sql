-- Phase 22-1: bound idempotency history without weakening replay semantics.
--
-- game_operations stores the exact response returned for an operation so that a
-- retry with the same operation_id can receive the same payload. Keeping that
-- contract means we intentionally retain the response JSON, but only for the
-- most recent 128 successful operations per user.

create index if not exists game_operations_user_revision_idx
  on public.game_operations(user_id, resulting_revision desc);

-- Trim historical rows that predate this retention policy. This only touches
-- idempotency history; the authoritative game_saves row is not modified.
with ranked_operations as (
  select
    operation.user_id,
    operation.operation_id,
    row_number() over (
      partition by operation.user_id
      order by
        operation.resulting_revision desc,
        operation.created_at desc,
        operation.operation_id desc
    ) as retention_rank
  from public.game_operations as operation
), stale_operations as (
  select ranked.user_id, ranked.operation_id
  from ranked_operations as ranked
  where ranked.retention_rank > 128
)
delete from public.game_operations as operation
using stale_operations as stale
where operation.user_id = stale.user_id
  and operation.operation_id = stale.operation_id;

-- Keep save mutation, idempotency insert, and pruning in one database function
-- invocation. The game_saves row lock serializes operations for a user, and any
-- failure while pruning rolls the whole operation back atomically.
...
begin
  ...
  -- The save-row lock above means there can be only one successful mutation for
  -- this user at a time. Pruning here therefore cannot race another operation
  -- for the same user and remains part of the same transaction as the save.
