-- Phase 34-1: reduce idempotency replay retention for long-session save stability.
--
-- Exact replay is only needed for immediate retries of ambiguous mutations.
-- Keeping 128 full responses multiplies the authoritative save payload by 128
-- for every active user. Retain the newest 16 successful operations instead.
-- The authoritative game_saves row is never pruned.
...
with ranked_operations as (
  ...
  where ranked.retention_rank > 16
)
...
begin
  ...
  values (
    ...
    v_resulting_revision, p_response
  );
...
begin
  ...
  values (
    p_user_id, btrim(p_operation_id), v_school_id, p_expected_revision,
    v_resulting_revision, v_response
  );
