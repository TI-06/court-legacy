-- Phase 34-2: stop storing full GameState snapshots in idempotency history.
--
-- game_saves remains the only authoritative full snapshot. game_operations keeps
-- only the operation id, resulting revision, and optional outcome required to
-- recognize/reconstruct an immediate retry.

create or replace function public.apply_game_operation_v3(
  p_user_id uuid,
  p_operation_id text,
...
revoke execute on function public.apply_game_operation_v3(
  uuid, text, bigint, jsonb, jsonb, jsonb
) from public, anon, authenticated;

grant execute on function public.apply_game_operation_v3(
  uuid, text, bigint, jsonb, jsonb, jsonb
) to service_role;

-- Existing rows can be compacted without changing their revision/idempotency identity.
