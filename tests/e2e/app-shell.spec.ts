...
test("@critical mobile team selection uses a court picker without overflow", async ({
  ...
  await expect(page.getByTestId("court-player").first()).toContainText(/調子 \d+/);
  await expect(page.getByTestId("court-player").first()).toContainText(/疲労 \d+/);
  await expect(page.getByTestId("bench-player").first()).toContainText(/調子 \d+/);
  await expect(page.getByTestId("bench-player").first()).toContainText(/疲労 \d+/);
